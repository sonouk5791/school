...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 for(const width of [1440,1024,768,390]){
  ...
  await page.locator('.ch-char-card').first().click();await page.locator('#btnModeFree').click();await page.locator('[data-cat=outfit]').click();
  const source=await page.locator('#charAvatar').getAttribute('src');
  await page.locator('#charAvatar').evaluate(i=>i.decode());
  assert.equal(await page.locator('#itemsLayer .ch-placed-item').count(),7);
  assert.equal(await page.locator('#itemsLayer svg').count(),7);
  const room=await page.locator('#roomViewport').boundingBox(),avatar=await page.locator('#customCharacterPreview').boundingBox();
  assert(avatar.height/room.height>=.43&&avatar.height/room.height<=.66);
  ...
  await page.getByRole('button',{name:'니트',exact:true}).click();
  assert.match(await page.locator('#avatarOutfitBadge').innerText(),/니트/);
  ...
  assert.equal(saved.placedItems.length,7);assert.equal(saved.customizations.kongi.outfit,'knit');
