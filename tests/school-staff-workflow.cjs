...
(async()=>{
 ...
 const enterStaff=async()=>{await page.locator('[data-school="mode"]').click();await page.locator('[data-school="setmode"][data-value="staff"]').click()};await enterStaff();
 const click=async(a,v)=>a==='nav'?page.locator(`[data-school="care"][data-value="${v}"]`).click():page.locator(`[data-action="${a}"]${v===undefined?'':`[data-value="${v}"]`}`).first().click();
 ...
 await page.reload();await enterStaff();await click('begin');await click('pre',id);
 ...
 await page.reload();await enterStaff();db=await page.evaluate(()=>JSON.parse(localStorage.getItem('digital_school_management_v1')));assert.equal(db.sessions.length,1);assert.equal(db.sessions[0].elder_id,id);assert.equal(db.sessions[0].aiReport,'사회복지사 확인 완료. 안전하게 참여하심.');
