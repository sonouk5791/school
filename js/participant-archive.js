...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  const defaults=window.CONSENT_FORMS.map(d=>({id:d.key,title:d.title}));
  ...
  async function render(message='') {
    ...
    if(view==='checks') {
      shell(common+`<p>기관에서 확인할 서류 목록입니다. 파일을 넣어도 확인 상태는 자동으로 바뀌지 않습니다.</p><p class="archive-summary">확인 완료 <b>${done} / ${docs.length}</b> · 담당자가 직접 확인해주세요.</p><form id="archiveChecks">${docs.map(d=>{
        ...
        return `<fieldset class="archive-check" data-document="${esc(d.id)}"><legend>${esc(d.title)}</legend><p>보관 파일 ${count}개${legacy.length?` · 작성한 서류 ${legacy.length}건`:''}</p><div class="archive-check-fields"><label class="care-field">확인 상태<select name="state">${store.states.map(state=>`<option${state===(c.state||'미확인')?' selected':''}>${state}</option>`).join('')}</select></label><label class="care-field">확인일<input name="date" type="date" value="${esc(c.date||'')}"></label></div><label class="care-field">보완 사항 · 원본 위치<input name="note" maxlength="1000" value="${esc(c.note||'')}" placeholder="예: 서명 원본은 사무실 2번 파일에 보관"></label>${defaults.some(x=>x.id===d.id)?button('양식 작성·확인','consent',d.id):''}</fieldset>`;
