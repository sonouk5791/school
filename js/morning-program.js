...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 function render(next){restoreVideo();stop();step=next;
  ...
  const titles=['안녕하세요!',`오늘은 ${date}이에요.`,'오늘 기분은 어떠세요?','이제 몸을 천천히 움직여볼까요?','이번에는 기억 놀이를 해볼까요?','오늘도 정말 잘하셨어요!'];
  const descriptions=['오늘도 즐겁게 시작해볼까요?','오늘 날씨는 어떨까요?','','무리하지 않아도 괜찮아요.<br>불편한 동작은 쉬어가셔도 됩니다.','','함께해서 즐거웠어요.'];
  ...
  if(step===0)window.VoiceManager?.speak({text:'안녕하세요. 오늘도 저와 함께 천천히 시작해볼까요?',speed:0.82,emotion:'warm'});
 }
