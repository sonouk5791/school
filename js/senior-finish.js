...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const classify = () => [...main.children].forEach(el => {
    ...
    groups.set(el, el === hero || el === start ? 'home' : el === warmup ? 'exercise' : el === friends ? 'friends' : el === today ? 'today' : el === history ? 'history' : 'activities');
  });
  const updateDate = () => {
