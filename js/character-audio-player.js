...
window.CharacterAudioPlayer=(()=>{
 ...
 function stop(){speaking=false;sequence++;audio.pause();audio.removeAttribute('src');audio.load();active=null;paint();}
 ...
 function scan(){if(active&&!visible(active.image))stop();
  ...
  const morning=document.querySelector('#morningProgram .morning-character');if(morning){const id=morning.getAttribute('src').match(/friend-(\w+)/)?.[1],step=Number(document.querySelector('.morning-progress')?.textContent.trim()[0]);attach(morning,id,step===6?'goodjob':step<=2?'welcome':'activity',morning);}
  ...
  const hero=document.querySelector('.hero-robot-wrapper'),heroImage=hero?.querySelector('img');if(heroImage)attach(hero,heroImage.getAttribute('src').match(/friend-(\w+)/)?.[1],'welcome',heroImage,true);
  paint();
 }
 async function refresh(){if(pending)return pending;pending=(async()=>{try{const response=await fetch('/character-recordings.json',{cache:'no-store'});const data=response.ok?await response.json():{},safe={};for(const id of Object.keys(names)){safe[id]={};for(const situation of ['welcome','activity','goodjob']){const url=data?.[id]?.[situation];if(typeof url==='string'&&new RegExp('^/public/audio/characters/'+id+'/'+situation+'\\.(mp3|wav)$').test(url))safe[id][situation]=url;}}files=safe;if(active&&files[active.id]?.[active.situation]!==audio.getAttribute('src'))stop();}catch{files={};stop();}finally{pending=null;scan();}})();return pending;}
