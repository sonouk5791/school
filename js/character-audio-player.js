...
window.CharacterAudioPlayer=(()=>{
 ...
 async function play(group,restart=false){if(window.CharacterVoice&&group){if(group.situation==='welcome')return CharacterVoice.playRecording(group.id,{restart});return speakAsCharacter(group.id,window.VoiceManager?.getCharacterGreeting(group.id)||('안녕하세요. '+names[group.id]+'예요.'),{restart});}const src=files[group?.id]?.[group?.situation];if(!src||!visible(group.el))return;
